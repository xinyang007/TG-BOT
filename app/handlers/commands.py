...
async def handle_commands(tid: int, admin_sender_id: int | str, text: str, conv_service: ConversationService):
    ...
    if cmd in commands_needing_conv_lookup:
         try:
             ...
             if not conv:
                  ...
                  await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "错误：此话题未关联对话实体，无法执行此命令。"})
             ...
             # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
             await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_thread_id": tid, "text": "处理命令失败：无法获取对话实体信息。"})
             return
    ...
    if cmd == "/close":
        ...
        try:
            ...
            # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
            await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": f"对话已标记为关闭。关联实体: {entity_type_in_topic} ID {entity_id_in_topic}"})
            logger.info(f"话题 {tid} (实体 {entity_type_in_topic} ID {entity_id_in_topic}) 由管理员 {admin_sender_id} 关闭")
        except Exception as e:
             logger.error(f"执行 /close 命令失败，话题 {tid}: {e}", exc_info=True)
             # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
             await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "关闭话题失败。"})
        ...
        if entity_type_in_topic != 'user':
             # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
             await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": f"错误：/ban 命令仅适用于用户对话，此话题关联实体类型为 {entity_type_in_topic} ID {entity_id_in_topic}。"})
             logger.warning(f"管理员 {admin_sender_id} 在话题 {tid} 中对非用户实体使用 /ban 命令 (类型: {entity_type_in_topic} ID: {entity_id_in_topic}).")
             return
        ...
        try:
            ...
            # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
            await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": f"用户 {entity_id_in_topic} 已被拉黑。"})
            logger.info(f"用户 {entity_id_in_topic} 在话题 {tid} 中由管理员 {admin_sender_id} 拉黑")
             ...
             # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
             await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "拉黑用户失败。"})
        ...
        if args:
            ...
            try:
                ...
                logger.debug(f"/unban 命令指定用户 ID: {user_id_to_unban}")
                 ...
                 # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
                 await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "无效的用户 ID。用法: /unban <用户ID> 或在用户对话话题中直接使用 /unban 解除当前用户拉黑。"})
                 return # 无效参数，停止处理
        else:
            ...
            if entity_type_in_topic != 'user':
                 ...
                 # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
                 await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": f"错误：/unban 命令仅适用于用户对话。此话题关联实体类型为 {entity_type_in_topic}。用法: /unban <用户ID> 或在用户对话话题中直接使用 /unban。"})
                 logger.warning(f"管理员 {admin_sender_id} 在话题 {tid} 中对非用户实体使用无参数 /unban (类型: {entity_type_in_topic}).")
                 return
        ...
        if user_id_to_unban is not None:
            try:
                ...
                # 管理员反馈
                # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
                await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": f"用户 {user_id_to_unban} 已被解除拉黑。"})
                ...
                # 用户通知在 service.unban_user 中完成
                 ...
                 # 管理员反馈
                 # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
                 await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "解除拉黑失败。"})
    ...
    #     pass # 此分支已被移除
        ...
        if entity_type_in_topic != 'user':
             # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
             await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": f"错误：/setlang 命令仅适用于用户对话，此话题关联实体类型为 {entity_type_in_topic} ID {entity_id_in_topic}。"})
             logger.warning(f"管理员 {admin_sender_id} 在话题 {tid} 中对非用户实体使用 /setlang 命令 (类型: {entity_type_in_topic} ID: {entity_id_in_topic}).")
             return

        if not args:
             # 管理员反馈：没有参数
             # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
             await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "用法: /setlang <语言码> (例如: en, zh-CN, fr)"})
        ...
        try:
            ...
            # Service notifies user. Just confirm to admin.
            # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
            await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": f"用户 {entity_id_in_topic} 的目标语言已更新为: {new_lang}"})
            logger.info(f"话题 {tid} (用户 {entity_id_in_topic}) 语言设置为 '{new_lang}'，由管理员 {admin_sender_id} 执行")
        except Exception as e:
             logger.error(f"执行 /setlang 命令失败，话题 {tid}: {e}", exc_info=True)
             # 管理员反馈：异常时给出反馈
             # 修正 chat_id 参数，使用 settings.SUPPORT_GROUP_ID
             await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "设置语言失败。"})
         ...
         await tg("sendMessage", {"chat_id": settings.SUPPORT_GROUP_ID, "message_thread_id": tid, "text": "未知命令,未发送给客户。\n可用命令: /close, /ban, /unban [<用户ID>], /setlang <语言码>"})
